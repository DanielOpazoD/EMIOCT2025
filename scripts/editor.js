...
export async function initializeEditor() {
      ...
      const FLOATING_NOTES_PRINT_FALLBACK_PAGE_SIZE = 'A4';
      const FLOATING_NOTES_PRINT_DEFAULT_MARGIN_MM = 12;
      ...
      function printVisibleFloatingNotes() {
        ...
        wrapper.style.width = `${Math.round(layerRect.width)}px`;
        wrapper.style.height = `${Math.round(layerRect.height)}px`;
        ...
        if (!Number.isFinite(pageWidthPx) || !Number.isFinite(pageHeightPx)) {
          const fallbackSpec = FLOATING_NOTES_PRINT_PAGE_DIMENSIONS_MM[FLOATING_NOTES_PRINT_FALLBACK_PAGE_SIZE];
          if (fallbackSpec) {
            const shortSide = Math.min(fallbackSpec.width, fallbackSpec.height);
            const longSide = Math.max(fallbackSpec.width, fallbackSpec.height);
            const fallbackOrientation = FLOATING_NOTES_PRINT_ALLOWED_ORIENTATIONS.includes(orientationSetting)
              ? orientationSetting
              : (layerRect.width >= layerRect.height ? 'landscape' : 'portrait');
            const widthMillimetres = fallbackOrientation === 'landscape' ? longSide : shortSide;
            const heightMillimetres = fallbackOrientation === 'landscape' ? shortSide : longSide;
            pageWidthPx = widthMillimetres * pxPerMillimetre;
            pageHeightPx = heightMillimetres * pxPerMillimetre;
          }
        }
        ...
        if (
          ...
          const marginPx = pageSpec ? 0 : FLOATING_NOTES_PRINT_DEFAULT_MARGIN_MM * pxPerMillimetre;
          const printableWidthPx = Math.max(pageWidthPx - (marginPx * 2), 0);
          const printableHeightPx = Math.max(pageHeightPx - (marginPx * 2), 0);
          if (printableWidthPx > 0 && printableHeightPx > 0) {
            const scaleX = printableWidthPx / layerRect.width;
            const scaleY = printableHeightPx / layerRect.height;
        ...
        printContainer.style.setProperty('--floating-notes-print-padding-x', `${Math.round(paddingXPx)}px`);
        printContainer.style.setProperty('--floating-notes-print-padding-y', `${Math.round(paddingYPx)}px`);
